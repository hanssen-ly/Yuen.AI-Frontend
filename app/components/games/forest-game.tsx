...
import { TreePine, Volume2, VolumeX, CirclePlay, CirclePause, Circle } from "lucide-react";
...
export function ForestGame() {
    ...
    useEffect(() => {
        ...
        if (isPlaying && elapsedTime < MEDITATION_DURATION) {
            timer = setInterval(() => {
                setElapsedTime((prev) => {
                    const newTime = prev + 1;
                    setProgress((newTime / MEDITATION_DURATION) * 100);
                    return newTime;
                });
            }, 1000);
        }
